...
from utils.data_prep import data_split_with_sampling_mining, data_split_with_sampling, check_data_leakage
...
def main():
    ...
    features = [
        "v_vel", "a_long", "v_lat", "v_lat_lag_5", "v_lat_lag_10", "v_lat_accel", "lat_dist_moved_15f",
        "can_go_right", "can_go_left", "is_special_lane", 
        "TTC", "actual_gap", "gap_rate_of_change", "traffic_pressure", "rel_speed"
    ]
    
    # 2. Train/Test Split
    # We use your sampling function here. 
    # Note: Hard mining can be integrated into data_split_with_sampling directly if desired.
    X_train, X_test, y_train, y_test = data_split_with_sampling(df, features, sampling_keep_factor=15)
