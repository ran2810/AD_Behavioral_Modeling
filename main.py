...
from utils.data_prep import data_split_with_sampling, check_data_leakage, lstm_train_test_split
...
def trigger_lstm(df, features, train_label='ngsim', window_size=30, save_benchmark=True):
    df_train, df_test = lstm_train_test_split(df, features)

    X_train = df_train[features]; y_train = df_train['lane_change']
    X_test  = df_test[features];  y_test  = df_test['lane_change']

    train_dataset = LaneChangeSequenceDataset(X_train, y_train, df_train['Vehicle_Global_ID'], window_size)
    test_dataset  = LaneChangeSequenceDataset(X_test,  y_test,  df_test['Vehicle_Global_ID'],  window_size)

    # Cap epoch to 6x minority so CPU training stays under ~30 min
    ...
    print(f"Epoch size capped at {epoch_size:,} sequences ({epoch_size // 64:,} batches/epoch)")
    ...
    for epoch in epoch_bar:
        ...
        epoch_bar.set_postfix(loss=f"{avg_loss:.4f}", left=f"{f1_l:.3f}", right=f"{f1_r:.3f}")
        tqdm.write(f"Epoch {epoch+1:2d} | Loss: {avg_loss:.4f} | Left F1: {f1_l:.4f} | Right F1: {f1_r:.4f}")

        if f1_r > best_right_f1:
            best_right_f1 = f1_r
            torch.save({'model': model.state_dict(), 'thresholds': thresholds},
                       f"best_lstm_{train_label}.pth")
...
def eval_lstm_on_dataset(model, df_test, features, train_label, test_label, device, window_size=30):
    df_sorted = df_test.sort_values(['Vehicle_Global_ID', 'Frame_Global_ID'])
    test_dataset = LaneChangeSequenceDataset(df_sorted[features], df_sorted['lane_change'],
                                             df_sorted['Vehicle_Global_ID'], window_size)
    test_loader  = DataLoader(test_dataset, batch_size=64, shuffle=False)
...
def run_cross_eval(df_ngsim, df_highd, features, model_name):
    ...
    if model_name in ('rf', 'both'):
        ...
        eval_rf_on_dataset(rf_ngsim, df_highd, features, 'ngsim', 'highd')
        ...
        eval_rf_on_dataset(rf_highd, df_ngsim, features, 'highd', 'ngsim')
...
if __name__ == "__main__":
    ...
    if args.cross_eval:
        if df_ngsim is None or df_highd is None:
            raise RuntimeError("--cross_eval requires both datasets.")
